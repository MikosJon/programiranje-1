...
def najdaljse_narascajoce_podazporedje(li):
    ...
    def podzaporedje(i, zadnji):
        ...
        current = li[i]
        if current >= zadnji:
            take = [current] + podzaporedje(i + 1, current)
            not_take = podzaporedje(i + 1, zadnji)
...
def zabica(mocvara):
    ...
    @lru_cache(maxsize=None)
    def skoki(i, energija):
        if i >= len(mocvara): return 0
        else:
            nova_energija = energija + mocvara[i]
            return 1 + min(skoki(i + j, nova_energija - j) for j in range(1, nova_energija + 1))
